...
static void memcpyAsync(std::vector<BenchParams> params,
                        unsigned long long sizeInElement,
                        unsigned int numThreadPerBlock,
                        unsigned long long *bandwidth, bool stride,
  ...
  CU_ASSERT(cuMemHostAlloc((void **)&blockingVar, sizeof(*blockingVar),
  ...
  *bandwidth = 0;
  ...
  for (unsigned i = 0; i < params.size(); ++i) {
    ...
    CU_ASSERT(cuDeviceGetAttribute(&multiProcessorCount,
                                   CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
                                   device));
    unsigned long long totalThreadCount =
        (unsigned long long)(multiProcessorCount * numThreadPerBlock);
    adjustedSizeInElement[i] =
        totalThreadCount * (sizeInElement / totalThreadCount);
  ...
  *blockingVar = 1;
  ...
  for (unsigned i = 0; i < params.size(); ++i) {
    ...
    *bandwidth += (adjustedSizeInElement[i] * sizeof(int) * loopCount *
                   1000ull * 1000ull) /
                  elapsedWithEventsInUs; // Bandwidth in Bytes per second

    CU_ASSERT(cuMemcpy(
        (CUdeviceptr)(((int *)params[i].dst) + adjustedSizeInElement[i]),
...
static void memcpyAsync_and_check(
    std::vector<BenchParams> params, unsigned long long sizeInElement,
  ...
  CU_ASSERT(cuDeviceGetAttribute(
      &kernelTimeout, CU_DEVICE_ATTRIBUTE_KERNEL_EXEC_TIMEOUT, device));
  if (kernelTimeout) {
    double timeout = 1.8;
    unsigned long long expectedBandwidth = 0;
    unsigned long long smallSizeInElement =
        sizeInElement > 1024 * 1024 * 128 ? 1024 * 1024 * 128 : sizeInElement;

    for (unsigned i = 0; i < params.size(); ++i) {
      memset_pattern(params[i].src, smallSizeInElement * sizeof(int),
                     0xCAFEBABE);
      memset_pattern(params[i].dst, smallSizeInElement * sizeof(int),
                     0xBAADF00D);
    }

    memcpyAsync(params, smallSizeInElement, numThreadPerBlock,
                &expectedBandwidth, stride, 1);
    for (unsigned i = 0; i < params.size(); ++i) {
      memcmp_pattern(params[i].dst, smallSizeInElement * sizeof(int),
                     0xCAFEBABE);
    }

    unsigned long long maxBytes =
        (unsigned long long)((double)expectedBandwidth * timeout * 0.25);
  ...
  memcpyAsync(params, sizeInElement, numThreadPerBlock, bandwidth, stride,
              loopCount);
  for (unsigned i = 0; i < params.size(); ++i) {
...
void launch_HtoD_memcpy_SM(const std::string &test_name,
  ...
  for (int currentDevice = 0; currentDevice < deviceCount; currentDevice++) {
    ...
    for (unsigned int n = 0; n < loopCount; n++) {
      memcpyAsync_and_check(params, size / sizeof(int),
                            (unsigned int)num_threads_per_sm,
                            &bandwidth_current, stride, loopCount);
      bandwidth_sum += bandwidth_current;
    }
    bandwidth_sum /= loopCount;

    if (bandwidth_sum > device_current_bandwidth)
      device_current_bandwidth = bandwidth_sum;
    //
  ...
  printIndexVector(std::cout << std::fixed << std::setprecision(2),
                   bandwidthValues);
}
...
static void launch_DtoD_memcpy_SM(const std::string &test_name, bool read,
  ...
  for (int currentDevice = 0; currentDevice < deviceCount; currentDevice++) {
    ...
    for (int peer = 0; peer < deviceCount; peer++) {
      ...
      if (canAccessPeer) {
        ...
        std::vector<BenchParams> params;
        if (read)
          params.push_back(BenchParams(srcBuffer, peerBuffer, srcCtx));
        else
          params.push_back(BenchParams(peerBuffer, srcBuffer, srcCtx));
  ...
  std::cout << "memcpy SM GPU(row) " << (read ? "->" : "<-")
            << " GPU(column) bandwidth (GB/s):" << std::endl;
  std::cout << std::fixed << std::setprecision(2) << bandwidth_matrix
            << std::endl;
}

static void launch_DtoD_memcpy_bidirectional_SM(const std::string &test_name,
                                                bool read,
                                                unsigned long long size,
                                                unsigned long long loopCount) {
  void *gpuAbuffer0;
