...
unsigned int NvBandwidthJson::addTestcaseResults(const PeerValueMatrix<double> &matrix, const std::string &description) {
    if (!m_root[NVB_TITLE][NVB_TESTCASES].isArray()) {
        return -1;
    } else if (m_root[NVB_TITLE][NVB_TESTCASES].size() < 1) {
        return -1;
    }
...
void NvBandwidthJson::addTestcase(const std::string &name, const std::string &status) {
    Json::Value testcase;
...
void NvBandwidthJson::recordErrorCurrentTest(const std::string &error) {
    ...
    if (m_root[NVB_TESTCASES].isArray()) {
        ...
        if (size > 0) {
            testcases[size-1][NVB_TESTCASE_ERROR] = error;
            testCaseExists = true;
        }
    }
...
void NvBandwidthJson::addTestcaseIfNeeded(const std::string &name, const std::string &status) {
    bool testCaseExists = false;
    if (m_root[NVB_TESTCASES].isArray()) {
        Json::Value &testcases = m_root[NVB_TITLE][NVB_TESTCASES];
        unsigned int size = testcases.size();
