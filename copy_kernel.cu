...
__global__ void stridingMemcpyKernel(unsigned int totalThreadCount, uint4* dst, uint4* src, unsigned long long chunkSizeInElement) {
...
CUresult copyKernel(CUdeviceptr dstBuffer, CUdeviceptr srcBuffer, size_t sizeInElement, CUstream stream) {
    ...
    stridingMemcpyKernel<<<gridDim, blockDim, sharedMemoryBytes, stream>>> (totalThreadCount, dst, src, chunkSizeInElement);
    return CUDA_SUCCESS;
}
